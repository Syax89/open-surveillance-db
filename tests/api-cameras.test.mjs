...
// ---------------------------------------------------------------------------
// POST /api/cameras — write API keys (EPIC api-keys T13)
// ---------------------------------------------------------------------------
// The route gates on requireWriteAuth(request, "submit"), so a machine
// client authenticates with `Authorization: Bearer <key>` (D4 `submit`
// scope) instead of a session cookie — no CSRF, no same-origin (T13: the
...
/** POST /api/cameras authenticating with a Bearer API key (no session). */
...
const keyPayload = { title: "Key camera", kind: "Dome", latitude: 45.0, longitude: 9.0 };

test("POST /api/cameras with a valid Bearer key (submit scope) stores the report under the key owner (T13)", async () => {
  liveKey();
  stub("createCamera", async (input) => ({ id: 20, ...input }));
  const { POST } = await camerasRoute();
  // No session cookie, no CSRF headers: the key alone authenticates.
  const response = await POST(bearerPost(keyPayload));

  assert.equal(response.status, 201);
  assert.equal(callArgs("createCamera")[0][0].contributorId, 7, "attribution is the key owner, never anonymous");
...
test("POST /api/cameras key path skips same-origin and CSRF (T13 conditional gate)", async () => {
  ...
  liveKey();
  stub("createCamera", async (input) => ({ id: 21, ...input }));
  const { POST } = await camerasRoute();
  const response = await POST(
  ...
  assert.equal(response.status, 201);
  assert.equal(callArgs("createCamera").length, 1);
  assert.notEqual((await responseBody(response)).error, "Cross-site request rejected. Refresh the page and try again.");
});

test("POST /api/cameras rejects a key without the submit scope (403 canonical, no write)", async () => {
  ...
  assert.equal(callArgs("createCamera").length, 0, "no write on scope mismatch");
});

test("POST /api/cameras rejects an invalid/revoked/expired key (401 canonical, no session fallback)", async () => {
...
test("POST /api/cameras key-authenticated requests are double-counted against their key:<id> bucket (D8/T13)", async () => {
  ...
  liveKey();
  stub("createCamera", async (input) => ({ id: 22, ...input }));
  const { env: workerEnv } = await loadTreeModule("cloudflare-workers.mjs");
  ...
  try {
    ...
    assert.equal((await responseBody(second)).error, "Too many submissions. Please try again shortly.");
    assert.equal(callArgs("createCamera").length, 1, "no write on the blocked request");
...
test("POST /api/cameras session requests consume ONLY the per-IP bucket (no per-key double-count, D8)", async () => {
  ...
  stub("createCamera", async (input) => ({ id: 23, ...input }));
  const { env: workerEnv } = await loadTreeModule("cloudflare-workers.mjs");
  const bindingCalls = [];
  workerEnv.WRITE_LIMITER = {
    async limit({ key }) {
