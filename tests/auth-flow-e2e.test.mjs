...
async function contributorSessionHeaders() {
  ...
  // The appeals filing route attributes the session to the `users` role
  // identity via the EXPLICIT users.contributor_id link (audit t_5ca60ab2,
  // P2 — the email bridge is gone). Link the fresh contributor to the demo
  // "Demo Contributor" users row (id 6) exactly as a deploy provisions real
  // accounts before opening the DB.
