...
export async function listPublicCameras(): Promise<CameraRecord[]> {
  const d1 = await getD1();
  const result = await d1.prepare("SELECT id, title, kind, CASE WHEN publish_manufacturer = 1 THEN manufacturer ELSE NULL END AS manufacturer, CASE WHEN publish_observed_on = 1 THEN observed_on ELSE NULL END AS observedOn, publish_manufacturer AS publishManufacturer, publish_observed_on AS publishObservedOn, address, notes, latitude, longitude, status, source, updated, description, created_at AS createdAt FROM cameras WHERE status IN ('verified', 'demo') ORDER BY id DESC").all<CameraRecord>();
  return result.results;
}
export type NearbyPublicCameraRecord = CameraRecord & { distanceMeters: number };
