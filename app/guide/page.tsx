  ...
  return <main id="main-content" className="record-page">
    ...
    <section className="records-section" aria-labelledby="status-title">
      ...
      <div className="record-list">
        <article className="record-list-card"><div><p className="card-topline"><span className="status-dot verified" /> {statusLabel(locale, "verified")}</p><h3>{t.verifiedTitle}</h3><p className="record-kind">{t.verifiedBody}</p></div></article>
        <article className="record-list-card"><div><p className="card-topline"><span className="status-dot needs-review" /> {statusLabel(locale, "needs_review")}</p><h3>{t.reviewTitle}</h3><p className="record-kind">{t.reviewBody}</p></div></article>
        <article className="record-list-card"><div><p className="card-topline"><span className="status-dot community-report" /> {statusLabel(locale, "pending")}</p><h3>{t.pendingTitle}</h3><p className="record-kind">{t.pendingBody}</p></div></article>
      </div>
    </section>
...
const english = {
  ...
  statusEyebrow: "Reading the records", statusTitle: "Each status says what the record can support.", statusIntro: "A status describes the current review state, not a guarantee that a camera is active, complete or permanently accurate.", verifiedTitle: "Reviewed and publishable", verifiedBody: "A moderator has approved this record for public display. It can appear in the map, directory and GeoJSON download.", reviewTitle: "Temporarily withheld", reviewBody: "Something needs checking: the record may be old, unclear, duplicated or the subject of a correction. It is not a public record while under review.", pendingTitle: "Awaiting a decision", pendingBody: "A submission has been saved for local review but is not visible in public data, the map, the directory or exports.",
...
const italian: { [K in keyof typeof english]: string } = {
  ...
  statusEyebrow: "Leggere i record", statusTitle: "Ogni stato chiarisce cosa può sostenere un record.", statusIntro: "Lo stato descrive la condizione corrente della revisione, non garantisce che una telecamera sia attiva, completa o accurata in modo permanente.", verifiedTitle: "Revisionata e pubblicabile", verifiedBody: "Un moderatore ha approvato il record per la visualizzazione pubblica. Può comparire in mappa, elenco e download GeoJSON.", reviewTitle: "Temporaneamente non visibile", reviewBody: "Qualcosa deve essere verificato: il record può essere vecchio, poco chiaro, duplicato o oggetto di una correzione. Non è pubblico durante la revisione.", pendingTitle: "In attesa di una decisione", pendingBody: "Una segnalazione è stata salvata per la revisione locale, ma non è visibile nei dati pubblici, nella mappa, nell’elenco o nelle esportazioni.",
