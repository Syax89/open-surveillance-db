...
type Props = {
  ...
  reportHref?: string;
  /**
   * P1-5 (F5): tool pages own the page header via .tool-heading (h1). When
   * embedded in a tool page the directory must not repeat eyebrow + h2 +
   * intro (FRONTEND_DESIGN §2.2). The "Use the map instead" cross-link
   * stays: toolbar action, not a heading.
   */
  showHeading?: boolean;
};
 ...
 * bundle; reused by the home page (anchor fallback) and by /directory.
 */
export function PublicDirectory({ filteredRecords, cameraKinds, search, setSearch, kindFilter, setKindFilter, freshnessFilter, setFreshnessFilter, setFreshnessCutoff, sortOrder, setSortOrder, showRecordOnMap, setCoordinates, onResetFilters, mapHref = "#map", reportHref = "#report", showHeading = true }: Props) {
