...
import { useReportFlow, type NearbyCandidate } from "../../lib/useReportFlow";
...
export { useReportFlow, type NearbyCandidate };
...
type Props = {
  ...
  /**
   * t_ebbe0ea3: map-click position picking. When provided, step 1 renders
   * the interactive mini-map (a map click = selectCoordinates, so the
   * coordinate readout and the reverse-geocode prefill update); the manual
   * coordinate fields below stay as the accessible fallback.
   */
  onSelectCoordinates?: (latitude: number, longitude: number) => void;
  /** Browser geolocation becomes available only after hydration; when it is,
   * this one-tap action feeds the exact same coordinate path as a map click. */
  geolocationAvailable?: boolean;
  geolocating?: boolean;
  geolocationNotice?: string;
  requestMyPosition?: () => void;
  ...
  showHeading?: boolean;
  /**
   * The report tool owns a full page heading. Its guidance and form must use
   * one vertical reading flow on desktop rather than the reusable split
   * section used by legacy/info embeddings.
   */
  layout?: "default" | "tool";
};

/**
 * Home report section: choose/enter a position, fill the report and submit
 * it for moderation. State and handlers come from `useReportFlow` via props.
 */
export function ReportForm({ coordinates, manualLatitude, setManualLatitude, manualLongitude, setManualLongitude, nearbyCandidates, nearbyLoading, nearbyError, duplicateConfirmationRequired, duplicateConfirmed, setDuplicateConfirmed, selectManualCoordinates, submitReport, onSelectCoordinates, kind = "", setKind = () => {}, direction = null, setDirection = () => {}, directionKnown = false, setDirectionKnown = () => {}, address = "", setAddress = () => {}, reverseGeocoding = false, geolocationAvailable = false, geolocating = false, geolocationNotice = "", requestMyPosition = () => {}, showHeading = true, layout = "default" }: Props) {
