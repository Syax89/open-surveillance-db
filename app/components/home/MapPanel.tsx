...
/**
 * Home map section: interactive map, selected-record card, loading
 * notice and data-export actions.
 */
export function MapPanel({ filteredRecords, selectedId, onSelect, onPick, coordinates, selectedCamera, loading, notice }: Props) {
  ...
  return (
    <section className="map-section" id="map" aria-labelledby="map-title">
      ...
      <div className="live-map-workspace"><div className="map-panel"><SurveillanceMap cameras={filteredRecords} selectedId={selectedId} focusLocation={coordinates} onSelect={onSelect} onPick={onPick} /><div className="map-hint">{t.mapHint}</div></div>
