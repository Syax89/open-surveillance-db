...
export default function Home() {
  ...
  return <main id="main-content">
    ...
    <section className="map-section" id="map" aria-labelledby="map-title">
      ...
      {selectedCamera && <article className="camera-card" aria-live="polite"><div className="card-topline"><span className={`status-dot ${selectedCamera.status}`} /> {statuses[selectedCamera.status] ?? selectedCamera.status}</div><h3>{selectedCamera.title}</h3><p>{selectedCamera.kind}</p><dl><div><dt>{t.source}</dt><dd>{selectedCamera.source}</dd></div><div><dt>{t.freshness}</dt><dd>{selectedCamera.updated}</dd></div><div><dt>{t.location}</dt><dd>{selectedCamera.latitude.toFixed(5)}, {selectedCamera.longitude.toFixed(5)}</dd></div></dl><p className="record-description">{selectedCamera.description}</p><a className="text-button" href="#correction">{t.reportIssue} <span aria-hidden="true">→</span></a></article>}</div>
