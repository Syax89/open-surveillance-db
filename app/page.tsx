...
export default function Home() {
  ...
  return <main id="main-content">
    ...
    <section className="map-section" id="map" aria-labelledby="map-title">
      ...
      {selectedCamera && <article className="camera-card" aria-live="polite"><div className="card-topline"><span className={`status-dot ${selectedCamera.status}`} /> {statusLabel(locale, selectedCamera.status)}</div><h3>{selectedCamera.title}</h3><p>{selectedCamera.kind}</p><dl><div><dt>{t.source}</dt><dd>{selectedCamera.source}</dd></div><div><dt>{t.freshness}</dt><dd>{selectedCamera.updated}</dd></div><div><dt>{t.location}</dt><dd>{selectedCamera.latitude.toFixed(5)}, {selectedCamera.longitude.toFixed(5)}</dd></div></dl><p className="record-description">{selectedCamera.description}</p><a className="text-button" href="#correction">{t.reportIssue} <span aria-hidden="true">→</span></a></article>}</div>
...
const english = {
  ...
  livePrototype: "Live prototype", mapTitle: "Explore documented cameras", osmBaseMap: "OpenStreetMap base map", mapCoverageNote: "The map shows documented public records only; an empty area does not prove that no cameras are present.", prototypeMode: "Prototype mode.", prototypeBanner: "The base map is real OpenStreetMap data; the two visible camera pins are clearly labelled illustrative records. Click anywhere on the map to select a position for a report.", mapHint: "Click the map to choose a report position", source: "Source", freshness: "Freshness", location: "Location", reportIssue: "Report an issue", loadingRecords: "Loading the public record API…", downloadGeoJson: "Download GeoJSON", downloadCsv: "Download CSV", readDataPolicy: "Read the data policy",
