 ...
 */
async function fetchCamerasPage(offset: number, signal: AbortSignal): Promise<CamerasPage> {
  const response = await fetch(`/api/cameras?limit=${PAGE_LIMIT}&offset=${offset}`, { signal });
