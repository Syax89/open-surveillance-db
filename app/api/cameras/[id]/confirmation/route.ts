 ...
 * confirm rate-limit bucket, the additive per-key bucket (EPIC api-keys
 * D8/T12) and — on the session branch only — the IP-hash burst bucket (never
 * the raw IP).
 */
...
/**
 * Shared PUT/DELETE guards in the fixed order (ADR 0018 §2.6, wave 2):
 * urlTooLong -> confirm rate-limit bucket -> write gate (dual-path, 401/403)
 * -> session-only: same-origin + CSRF (403) and IP-hash burst (429) ->
 * additive per-key bucket (429, no-op for session) -> strict id parse (404).
 */
async function guardMutation(
  ...
  let gate;
  try {
    // Write gate (multi-method auth Fase E1 + EPIC api-keys T15): the toggle
    // requires a VERIFIED contributor, authenticated EITHER by a verified
    // session cookie OR by a private write API key carrying the `confirm`
    // scope (`Authorization: Bearer *** D4). Anonymous (401), unverified
    // (403) and scope-mismatch (403) share ONE single response body
    // (anti-enumeration, no-store). Raising the bar here is part of the
    // anti-gaming layer: an unverified account cannot mass-confirm records,
    // so a freshly registered (unverified) contributor cannot influence
    // public verification counts. (Decision point flagged in the E1 PR: the
    // email-verified requirement stacks on top of the existing level gate —
    // ≥1 verified contribution — in db/confirmations.ts.)
    gate = await requireWriteAuth(request, "confirm");
  } catch (error) {
  ...
  // Session-only extras (T15 wave 2): CSRF/same-origin and the IP-hash burst
  // bucket apply ONLY on the session branch. A machine client holding a
  // secret bearer credential carries no ambient authority from a browser
  // origin (no CSRF), and its toggle volume is already bounded by the
  // additive per-key `key:<id>` bucket below — so the burst bucket would be
  ...
  // whole story), so this check is a no-op for them — and for machine
  // clients it is exactly the bucket that covers the burst the session
  // branch gets from checkConfirmIpBurst.
  const keyLimit = await checkRateLimitForKeyAuth(env, "confirm", request, limitOptions, gate);
  if (!keyLimit.allowed) {
    console.warn("PUT/DELETE /api/cameras/[id]/confirmation rate limited (per-key bucket)");
    recordRateLimitBlock(env, {
      route: "/api/cameras/[id]/confirmation",
      key: keyLimit.key,
      windowSeconds: limitOptions.windowSeconds,
