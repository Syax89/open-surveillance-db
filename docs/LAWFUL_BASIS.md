...
- **Status:** draft outline for pre-launch review (ADR 0002/0003)
...
Not intentionally collected. Incidental capture in evidence (faces, plates, interiors) is prevented by policy (MODERATION.md) and enforced by redaction/deletion (RETENTION_SCHEDULE R6). If a future feature collects such data, a separate art. 9 assessment is required before implementation.
...
- The *Garante per la protezione dei dati personali* is the supervisory authority for complaints (see PRIVACY_NOTICE § 7).
...
- [ ] Confirm controller entity and contacts (PRIVACY_NOTICE § 1, 8).
