...
3. **Legal emergencies.** The only human write power over the **record lifecycle** is the administrator's legal-emergency hide or removal, used when the law requires it and reviewed retrospectively. Administrators cannot restore or un-hide unilaterally: the community consensus of § 5 is the only reversal path (ADR 0021 § 6/§ 8). The photo redaction gate (§ 5.5) is the only other human step and it acts on the **photo's own status** (`pending` → `approved`), never on the record lifecycle.
