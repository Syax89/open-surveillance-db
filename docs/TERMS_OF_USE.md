...
- [x] **Account erasure endpoint (R7):** implemented in PR #61 — `DELETE /api/auth/account` with atomic de-attribution of reports (`contributor_id = NULL`), session revocation and hard delete of the contributor (`eraseContributor` in `db/auth.ts`); tracked in RETENTION_SCHEDULE.md R7.
