...
| Impact on data subjects | **Lower than the pre-pivot community plan** (COMMUNITY_PLAN.md § 5, superseded): **no public profile, no attribution, no edit history** — public payloads expose **aggregates only** (counts/scores, never who acted — ADR 0021 § 3.5/§ 7); one action per user per record; no free text; actions carry no email, real name or IP-derived data |
...
- **Conclusion:** legitimate interest is balanced for the community-action purposes. The impact is *lower* than the pre-pivot community profile/verifications model (no public profile, no attribution, no edit authorship), so the § 3.1 conclusion — art. 6(1)(f) balanced for the publication purpose — holds **a fortiori** for community actions. Review together with § 3.1 on any material change.
