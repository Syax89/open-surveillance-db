...
| A2 | **Upload / submit:** `POST /api/cameras` crea solo record `pending` (ADR 0001); nessun endpoint di photo/evidence upload ("no photo upload yet" nei testi UI; nessuna route upload nel repo). | TERMS §4.1 ("evidence uploads are not enabled yet"); §5.1 (nessuna garanzia di pubblicazione, ingresso come `pending`). | ✅ Coerente | Nessuna discrepanza: l'assenza di upload corrisponde esattamente alla dichiarazione. |
...
6. **Account erasure endpoint (R7)** — PR #57 (auth contributori) non ha una route di cancellazione account; la FK `cameras.contributor_id` (senza ON DELETE) blocca l'hard delete di un contributor con report attribuiti. Da implementare con de-attribution (`contributor_id = NULL`) prima del lancio — owner Ada, tracciato in TERMS §15 e RETENTION_SCHEDULE R7. Non è un blocco legale: la policy (R7) è già definita e il diritto di erasure resta esercitabile via `privacy@opensurveillancedb` (deletion manuale/processo documentato), ma l'automazione è un requisito pre-launch.
