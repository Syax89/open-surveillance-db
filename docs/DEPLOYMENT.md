...
## Local test deployment (Proxmox LXC 114)

The always-on test site lives on Proxmox container **114 `osdb-test`**
(192.168.1.201:3000), reachable only on the LAN. This is the reference
environment for staging checks until the Cloudflare deployment is enabled.
...
- Source at `/opt/open-surveillance-db`, updated with `git fetch && git reset
  --hard origin/main`, then `npm ci`.
