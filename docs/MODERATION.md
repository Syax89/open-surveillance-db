...
The retired review cycle (pending → verified/rejected, appeals, edit moderation) is not part of this model; the moderation queue and appeal tables were closed by migration and survive only as history (ADR 0021 § 7.3, migration plan).
...
- **No transition happens on a timer** (ADR 0021 § 2.2): records are never moved to `needs_review`/`stale`; a record nobody confirms stays `active` until the community says otherwise.
...
There is no appeal workflow for photo decisions: the community-driven model replaces the appeal flow (ADR 0021 § 7), and the record itself remains challengeable through community actions or the private correction path.
