...
- **Status:** draft for pre-launch review (ADR 0003)
...
|---|-----------|---------|----------------|------|-----------------|----------------------|----------------|--------|
| PR1 | **Cloudflare, Inc.** | Cloudflare Workers (compute/edge) + D1 (database, binding `DB`) | All application data: pending/verified records, notes, evidence refs, moderation decisions, correction requests. No R2 in use (`hosting.json` `r2: null`). | **Processor** (art. 28) | Cloudflare Data Processing Addendum (DPA) incorporating **EU SCCs** (modules 2/3 as applicable) | Transfers EU→US possible via edge; mitigation: D1 configured with EU residency (**`weur` location hint**); encryption in transit (TLS); supplementary-measures assessment documented in § 2 | Per Cloudflare's published sub-processor list (DPA § list, change-notified) — reviewed at contracting | Onboarded (test) / **pre-launch: execute DPA + region pinning** |
...
### Cloudflare (PR1)
- **Transfer instrument:** Cloudflare DPA with **EU Standard Contractual Clauses** (2021/914) — controller-to-processor and processor-to-processor modules as applicable; DPA executed by the controller (placeholder entity) before launch.
...
- **Backup retention:** D1 automatic backups (hourly, 24 h) and Time Travel PITR (30 days) are accepted as the technical erasure horizon (RETENTION_SCHEDULE R10); no R2 export backup is configured.
