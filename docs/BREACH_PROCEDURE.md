...
- For published-data incidents: hide affected records immediately (MODERATION_SLA emergency flow) pending review.
...
Retention: **2 years** (aligned with the audit log, RETENTION_SCHEDULE R5/R9); the register is not public.
...
*Pre-launch note: this procedure becomes operational once the privacy mailbox and on-call rotation exist (see PRIVACY_NOTICE § 8 and GOVERNANCE.md "Before launch").*
